...
def pick(paragraphs, select, k) :
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    result=[r for r in paragraphs if select(r)]
    return result[k]
    # END PROBLEM 1
...
def about(keywords):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def about_something(para):
        para=remove_punctuation(lower(para))
        words=split(para)
        for rest in keywords:
            if rest in words:
                return True
        return False 
    return about_something
    # END PROBLEM 2
...
def accuracy(typed: str, source: str) -> float:
    ...
    if n==0:
        if source_words==[]:
            correct=1
    while i<n and i<len(source_words):
...
def autocorrect(typed_word: str, word_list: list[str], diff_function, limit: int) -> str:
    ...
    diff_key=lambda x:return diff_function(typed_word,x,limit)
    min_word=min(word_list,key=diff_key)
    if diff_key(min_word)>limit:
        return typed_word
    else:
        return min_word
    # END PROBLEM 5
...
def furry_fixes(typed: str, source: str, limit: int) -> int:
    ...
    return furry_fixes(typed[1:],source[1:],limit-diff)+diff
    # END PROBLEM 6
...
def minimum_mewtations(typed: str, source: str, limit: int) -> int:
    ...
    if typed[0]==source[0]:
        ...
        substitute = minimum_mewtations(source[0]+typed[1:],source-1)
        # BEGIN
        "*** YOUR CODE HERE ***"
...
def report_progress(typed: list[str], source: list[str], user_id: int, upload) -> float:
    ...
    for i in range(min(len(typed),len(source))):
        if typed[i]!=source[i]:
            break
    return {"id":user_id,"progress":i+1/len(source) }
    # END PROBLEM 8
